...
class InstagramDailyPostAutomation:
    ...
    async def login_instagram_with_2fa(self, page, username, password, account_number):
        """Enhanced login with automatic 2FA handling"""
        try:
            ...
            self.log(f"[Account {account_number}] 📱 Navigated to Instagram login page")
            await self.human_delay(2000, 4000)
            
            # Handle cookie consent
            try:
                cookie_button = await page.wait_for_selector('button:has-text("Allow all cookies")', timeout=5000)
                await cookie_button.click()
                self.log(f"[Account {account_number}] 🍪 Accepted cookies consent")
                await self.human_delay(1000, 2000)
            except:
                self.log(f"[Account {account_number}] ℹ️ No cookie consent dialog found")
            ...
            self.log(f"[Account {account_number}] 🔑 Submitted login form")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if "accounts/onetap" in current_url:
                        ...
                        # Add timeout for dialog handling to prevent infinite loops
                        try:
                            dialog_handled = await asyncio.wait_for(
                                self.handle_login_info_save_dialog(page, account_number),
                                timeout=60.0  # 60 second timeout instead of 30
                            )
                            
                            if dialog_handled:
                                self.log(f"[Account {account_number}] ✅ Save login dialog handled successfully!")
                                # Wait a bit and check final URL
                                await self.human_delay(2000, 3000)
                                final_url = page.url
                                
                                if "accounts/onetap" not in final_url:
                                    if final_url == "https://www.instagram.com/" or final_url.startswith("https://www.instagram.com/?"):
                                        ...
                                        # Try to navigate to home page manually
                                        self.log(f"[Account {account_number}] 🔄 Not on home page, navigating manually...")
                                        try:
                                            await page.goto("https://www.instagram.com/", wait_until="domcontentloaded", timeout=15000)
                                            await self.human_delay(2000, 3000)
                                            final_url = page.url
                                            if final_url == "https://www.instagram.com/" or final_url.startswith("https://www.instagram.com/?"):
                                                self.log(f"[Account {account_number}] ✅ Manual navigation successful!")
                                                await self.update_visual_status(page, "Login successful!", 5)
                                                return True
                                        except Exception as nav_error:
                                            self.log(f"[Account {account_number}] ⚠️ Manual navigation failed: {nav_error}")
                                else:
                                    self.log(f"[Account {account_number}] ⚠️ Still on save login page, but continuing anyway...")
                                    # Force continue to prevent infinite loop
                                    return True
                            else:
                                self.log(f"[Account {account_number}] ⚠️ Save login dialog handling failed, but continuing...")
                                return True  # Continue anyway to prevent infinite loop
                                
                        except asyncio.TimeoutError:
                            self.log(f"[Account {account_number}] ⏰ Save login dialog handling timed out after 60 seconds!", "WARNING")
                            self.log(f"[Account {account_number}] 🔄 Forcing continuation to prevent infinite loop...")
                            # Force continue to prevent the script from hanging
                            try:
                                await page.goto("https://www.instagram.com/", wait_until="domcontentloaded", timeout=10000)
                                await self.human_delay(2000, 3000)
                                self.log(f"[Account {account_number}] ✅ Forced navigation after timeout!")
                                await self.update_visual_status(page, "Timeout - forced continuation!", 5)
                                return True
                            except Exception as force_nav_error:
                                self.log(f"[Account {account_number}] ⚠️ Forced navigation also failed: {force_nav_error}")
                                # Still continue to prevent total failure
                                return True
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    ...
                    self.log(f"[Account {account_number}] ✅ Browser launched successfully")
                except Exception as e:
                    self.log(f"[Account {account_number}] ❌ Failed to launch browser: {e}", "ERROR")
                    return False
                ...
                try:
                    ...
                    self.log(f"[Account {account_number}] 🚀 Starting enhanced login process...")
                    self.log(f"[Account {account_number}] 📊 Current page URL before login: {page.url}")
