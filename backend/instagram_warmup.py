...
async def login_instagram_with_2fa(context: BrowserContext, username, password, totp_secret=None, log_callback=None):
    """
    Enhanced login function with automatic 2FA handling
    """
    page = await context.new_page()
    try:
        if log_callback:
            log_callback(f"[{username}] 🚀 Starting login process...")
        
        await page.goto("https://www.instagram.com/accounts/login/", wait_until="domcontentloaded")
        if log_callback:
            log_callback(f"[{username}] 📱 Navigated to Instagram login page")
        await human_like_delay()
        ...
        try:
            ...
            if log_callback:
                log_callback(f"[{username}] 🍪 Accepted cookies consent")
            await human_like_delay()
        except Exception:
            if log_callback:
                log_callback(f"[{username}] ℹ️ No cookie consent dialog found")

        # Enter credentials
        if log_callback:
            log_callback(f"[{username}] ⌨️ Entering login credentials...")
        ...
        if log_callback:
            log_callback(f"[{username}] 🔑 Submitted login form")
        await human_like_delay()

        # Check various post-login scenarios
        max_attempts = 3
...
async def login(context: BrowserContext, username, password, log_callback=None):
    ...
    return await login_instagram_with_2fa(context, username, password, totp_secret, log_callback)
